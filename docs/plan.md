...
**第一阶段（已完成）**：后端骨架 + 全部 MVP 接口真实可跑 + 10 个小程序页面真实联调（非假数据，SQLite 落库）。

**第一阶段补充（已完成）**：按老板确认的实际业务流程补齐三块——

1. **仓库称重计费**。运费改为按仓库实际称重算，不再用用户预报时自己填的重量
   （填 0.1kg 实寄 10kg 就白送了）。入库不录称重不让过，未称重的包裹不能下单。
2. **订单费用明细**。订单金额由若干条可解释的费用加总（运费 + 入库拍照 + 以后的
   打包费/加固费），用户端逐条展开。下单前预览和实际扣款走同一个计算函数。
3. **包裹拍照**。入库拍照是收费增值服务（预报时勾选或到仓后补申请，按包裹计费，
   **拍了才收钱**）；打包留底照免费，出纠纷时自证用。图片走独立的 `/upload` 端点。

**第二阶段（已完成）**：接入真实微信 AppID/Secret（`WX_APPID`/`WX_SECRET` 环境变量）。
两者配齐后 `devLogin` 自动禁用，不需要额外开关，避免有人忘记关掉它变成线上后门。

**第三阶段：上线过审**

部署这一半已经就绪，见 [`deploy.md`](deploy.md)：
- `deploy/` 下一份 docker compose，后端 + Caddy 两个容器，HTTPS 证书自动申请续期
  （微信小程序只允许请求 HTTPS 域名，这一步绕不过去）
- 生产环境启动自检：`JWT_SECRET` 仍是默认值 / `STAFF_KEY` 未设 / 微信凭证没配齐，
  任何一项不合格直接拒绝启动并说明怎么改
- `backend/tests/` 88 个用例 + GitHub Actions，改动不至于把主线改坏

剩下要你出面的部分：
- 买服务器 + 域名（国内服务器要备案，10-20 天，建议早办）
