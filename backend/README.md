...
**哪条线便宜要看具体重量**，不是一条恒定更贵：精致小每 0.5kg 跳 ¥35（跳得粗），
无忧草每 0.1kg 跳 ¥8（跳得细），所以刚跳完一档的那一段无忧草反而更划算——
实测 1.2–1.4kg、1.7–1.8kg、2.2–2.3kg、2.7kg 这几段是无忧草赢，其余精致小赢。
首页计算器按价格升序返回并把最便宜的那条标出来，用户不用自己比。

### 申报价值：默认只建议，不拦截

`min/max_declared_value` 是每条线**建议**的价值区间，显示给用户看。

是否强制拦截由 `business.ENFORCE_DECLARED_VALUE_LIMIT` 控制，**默认 False**：
实际清关查得不严，而且申报价值怎么算本身就没有一刀切的答案（整票还是单件、
怎么估值），硬拦只会把用户挡在门外、把客诉推给客服。

拦截逻辑留着并且有测试覆盖——物流商哪天开始卡了，把开关改成 `True` 就立刻生效。
开启后按**整票**申报价值算（各包裹之和，用户没填就退回商品价值，因为报 0 更容易
被查），超出区间就拦下并提示换线或拆单。
