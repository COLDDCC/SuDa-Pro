...
```
下单 → 待付款(pending) → 已付款待打包(paid) → 运输中(shipped) → 已签收(signed)
                ↓
            已关闭(closed)
```

**「已付款」这一步不能省。** 运费是线下收的（微信转账），系统收不到钱，所以必须
由客服手动确认。没有这道闸，仓库会照着待发货列表干活，把货发出去之后才发现钱没到，
只能追着客户要。

- `markShipped` 只接受 `paid` 状态的订单，`pending` 直接报错并说明原因
- `markPaid` 记下收款时间和备注（微信尾号之类），对账要查是哪笔钱
- `revertPaid` 可以撤回——点错一次就等于白发一单货
- 客户自己只能关掉**没付款**的订单；已付款的要退款，必须走客服

后台「待收款」和「待打包发货」是两个独立的 tab，各自只显示该状态的订单，
收完款自动从待收款里消失，不会重复收。
...
**待付款或已付款**的订单用了相同的身份证/电话/地址，直接拦住并说明原因。
两种状态都还没发出，都会挤在下一个航次里。
